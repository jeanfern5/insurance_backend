//Functions for creating client database
	...
	function (error, results, fields) {
		if (error) {
			if (error.code === "ER_DB_CREATE_EXISTS") {
				throw new Error(`Failed to create ${client_name} database because it already exists`);
			}else{
	...
	function (error, results, fields) {
		if (error) {
			if (error.code === "ER_TABLE_EXISTS_ERROR") {
				throw new Error(`Failed to create ${client_name}.CLAIMS table because it already exists`);
			} else if (error.code === "ER_BAD_DB_ERROR") {
				throw new Error(`Failed to create ${client_name}.CLAIMS table because database ${client_name} does not exist.`);
			} else {
				throw new Error(`Unhandled error: ${error}`);
			}
		} else {
