	...
	function (error, results, fields) {
		if (error) {
			if (error.code === "ER_DB_CREATE_EXISTS") {
				throw new Error(`Failed to create YaroDB database because it already exists.`);
			} else {
	...
	function (error, results, fields) {
		if (error) {
			if (error.code === "ER_TABLE_EXISTS_ERROR") {
				throw new Error('Failed to create YaroDB.YARO_USER table because it already exists');
			} else {
				throw new Error(`Unhandled error: ${error}`);
			}
		} else {
...
function createYaroClientTable(connection) {
	...
	function (error, results, fields) {
		if (error) {
			if (error.code === "ER_TABLE_EXISTS_ERROR") {
				throw new Error(`Failed to create YaroDB.YARO_CLIENT table because it already exists`);
			}else{
				throw new Error(`Unhandled error: ${error}`);
			}
		} else {
...
function createYaroActiveUserTable(connection) {
	...
	function (error, results, fields) {
		if (error) {
			if (error.code === "ER_TABLE_EXISTS_ERROR") {
				throw new Error ('Failed to create YaroDB.YARO_ACTIVE_USER table because it already exists');
			}else{
				throw new Error(`Unhandled error: ${error}`);
			}
		} else {
